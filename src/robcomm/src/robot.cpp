...
void Robot::handle_get_udp_protocol_version(MSG_GET_UDP_PROTOCOL_VERSION* msg)
{
  ...
  if (robot_protocol_version_major == 0 && robot_protocol_version_minor == 0)
  {
    ...
    {
      exception_ss << "Controller reports incompatible protocol version " << msg->major << "." << msg->minor;
      throw std::runtime_error(exception_ss.str());
    }
  ...
  else if (robot_protocol_version_major != msg->major || robot_protocol_version_minor != msg->minor)
  {
    exception_ss << "Controller changed protocol version from " << robot_protocol_version_major << "."
                 << robot_protocol_version_minor << " to " << msg->major << "." << msg->minor << "during operation!";
    throw std::runtime_error(exception_ss.str());
  }
}
...
{
  ssize_t n = sendto(sockfd_tx, msg, len_SET_MSG(msg), 0, (struct sockaddr*)&robot_addr, sizeof(robot_addr));
